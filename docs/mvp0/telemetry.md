...
> **日期**：2026-07-05
> **上游文档**：`spec.md`（v0.7）§10——本文只定义「怎么采集与计算」，测什么、多少算过以规格书 §10 为准。
> **配套文档**：`playtest-plan.md`（测试执行方案，消费本文导出的数据）、`formulas.md` §5（诊断规则编号的定义源）、`content.md` §2（地图/关卡/精英编号的定义源）、`economy.md`（声望结算字段与节点 ID 的定义源）。
> **范围纪律**：本埋点系统只服务 §10 的一次性设计验证，**不是**产品分析体系——纯本地记录（与存档同级持久化）、本地 JSON 一键导出、无服务端管线、无用户画像、无留存/漏斗分析。MVP-1 若需要长期数据体系，另立规格，勿在本文上扩建。
